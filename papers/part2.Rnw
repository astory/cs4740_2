...
and trigrams (\Sexpr{subset(results,unk=='Off'&smooth.gram=='On'&size==996759&n==2)$score}\%).
...
Unknown word handling with unks generally decreased performance. Whenever we turned
...
we've got a lot of instances of words that we only see once:  transforming them
to unks means that we don't see them at all, which reduces the ability of our
...
The increase in score was on the order of 10\% for every 1000\% increase
...
transforming the first occurence of a word into an unk amplifies the effect of
...
As a final note, there are many variables that we did not explore in our study
that could have improved performance.  For example, we did not determine the
effects of reducing all the input to lower case, or writing a series of
heuristics for choosing tags for words that we have not seen, without unking.
...
@
\end{center}
\caption{\label{fig:n}Score of predictions as a function of training corpus size and use of unknown word handling with smoothed bigram probabilities. ``BL'' refers to the baseline score.}
\end{figure}
